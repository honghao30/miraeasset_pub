function buildPad(target, txt) {
    ...
    let calbackFun = null; // 콜백 함수
    let numArray = ['⇐'];
    let box = [];

    const font = 'normal 35px gothic';
    const clickedColor = 'rgba(172, 231, 243, 0.4)';
    ...
    function _init(reff) {
        ...
        while (row < 3) { // 박스를 완성합니다.
            let column = 0;
            while (column < 4) {
                let x = WIDTH * column / 4;
                let y = HEIGHT * row / 3;
